...
class EEG_Processing_User {
  ...
  EEG_Processing_User(int NCHAN, float sample_rate_Hz) {
    ...
    //fftPlot = createWriter("C:/Git/ml-agents/unity-environment/Assets/ML-Agents/Alphawave/AlphaRecord.txt");
  }
...
fftPlot = createWriter("C:/Git/ml-agents/unity-environment/Assets/ML-Agents/Alphawave/AlphaRecordbackup.txt");
}
    ...
    for (int Ichan=0;Ichan < nchan; Ichan++) {
      ...
      for (int Ibin=0; Ibin < fftBuff[Ichan].specSize(); Ibin++){
        ...
        if((Ibin>7.5)&&(Ibin<15.5)){
          
        tempAlpha += FFT_value_uV;
        
        }
      }
    }
    
  fftPlot.print(tempAlpha);
  println(tempAlpha);
